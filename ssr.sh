...
Install_SSR(){
	...
	sed -i 's/python_ver="python"/python_ver="python3"/' /etc/init.d/ssr
	sed -i 's/collections.MutableMapping/collections.abc.MutableMapping/' /usr/local/shadowsocksr/shadowsocks/lru_cache.py
	Start_SSR
}
Update_SSR(){
	SSR_installation_status
...
Check_Libsodium_ver(){
	...
	Libsodiumr_ver=$(wget -qO- "https://github.com/jedisct1/libsodium/tags"|grep -oE 'tag/[0-9]+\.[0-9]+\.[0-9]+'|head -1|sed 's/tag\///')
